...
void ScreenSpaceReflectionApp::MinSampleNumHandler(KlayGE::UISlider const & sender)
{
	...
	if (teapot_)
	{
		checked_cast<ReflectMesh&>(teapot_->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).MinSamples(sample_num);
...
void ScreenSpaceReflectionApp::MaxSampleNumHandler(KlayGE::UISlider const & sender)
{
	...
	if (teapot_)
	{
		checked_cast<ReflectMesh&>(teapot_->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).MaxSamples(sample_num);
...
void ScreenSpaceReflectionApp::EnbleReflectionHandler(KlayGE::UICheckBox const & sender)
{
	...
	if (teapot_)
	{
		checked_cast<ReflectMesh&>(teapot_->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).EnbleReflection(enabled);
	}
}
...
uint32_t ScreenSpaceReflectionApp::DoUpdate(KlayGE::uint32_t pass)
{
	...
	if (0 == pass)
	{
		if (loading_percentage_ < 100)
		{
			...
			else if (loading_percentage_ < 40)
			{
				if (y_cube_->HWResourceReady() && c_cube_->HWResourceReady())
				{
					...
					checked_cast<ReflectMesh&>(teapot_->FirstComponentOfType<RenderableComponent>()->BoundRenderable())
						.SkyBox(y_cube_, c_cube_);
		...
		else
		{
			...
			auto& teapot_mesh = checked_cast<ReflectMesh&>(teapot_->FirstComponentOfType<RenderableComponent>()->BoundRenderable());
