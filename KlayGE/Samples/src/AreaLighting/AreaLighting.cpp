...
void AreaLightingApp::OnCreate()
{
	...
	}
	{
		...
		auto sphere_area_light_proxy_node = MakeSharedPtr<SceneNode>(SceneNode::SOA_Cullable | SceneNode::SOA_Moveable);
		sphere_area_light_proxy_node->TransformToParent(MathLib::scaling(0.1f, 0.1f, 0.1f));
		sphere_area_light_proxy_node->AddChild(LoadLightSourceProxyModel(sphere_area_light_)->RootNode());
		sphere_area_light_node_->AddChild(sphere_area_light_proxy_node);
	...
	}
	{
		...
		auto tube_area_light_proxy_node = MakeSharedPtr<SceneNode>(SceneNode::SOA_Cullable | SceneNode::SOA_Moveable);
		tube_area_light_proxy_node->TransformToParent(MathLib::scaling(0.1f, 0.1f, 0.1f));
		tube_area_light_proxy_node->AddChild(LoadLightSourceProxyModel(tube_area_light_)->RootNode());
		tube_area_light_node_->AddChild(tube_area_light_proxy_node);
...
void AreaLightingApp::RadiusChangedHandler(UISlider const & sender)
{
	...
	sphere_area_light_node_->Children()[0]->TransformToParent(MathLib::scaling(radius, radius, radius));
...
void AreaLightingApp::LengthChangedHandler(UISlider const & sender)
{
	...
	tube_area_light_node_->Children()[0]->TransformToParent(MathLib::scaling(0.1f, 0.1f, length));
