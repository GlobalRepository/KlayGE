...
uint32_t VideoTextureApp::DoUpdate(uint32_t /*pass*/)
{
	...
	auto& teapot_renderable = checked_cast<RenderTeapot&>(object_->FirstComponentOfType<RenderableComponent>()->BoundRenderable());
	teapot_renderable.VideoTexture(se.PresentTexture());
	teapot_renderable.LightPos(MathLib::transform_coord(float3(0, 0, 0), light_node_->TransformToParent()));
