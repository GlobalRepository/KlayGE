...
void SubSurfaceApp::OnCreate()
{
	...
	light_node_->TransformToParent(MathLib::translation(0.0f, 2.0f, -3.0f));
	light_node_->AddComponent(light_);
	light_node_->AddChild(light_proxy->RootNode());
	Context::Instance().SceneManagerInstance().SceneRootNode().AddChild(light_node_);
		...
		model_->ForEachMesh([this](Renderable& mesh)
			{
				...
				detailed_mesh.LightPos(MathLib::transform_coord(float3(0, 0, 0), light_node_->TransformToParent()));
