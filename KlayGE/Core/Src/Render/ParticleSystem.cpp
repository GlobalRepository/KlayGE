...
namespace KlayGE
{
	...
	void ParticleSystem::UpdateParticlesNoLock(float elapsed_time)
	{
		...
		if (!actived_particles_.empty())
		{
			...
			checked_cast<RenderParticles&>(this->FirstComponentOfType<RenderableComponent>()->BoundRenderable())
				.PosBound(AABBox(min_bb, max_bb));
		}
	}
	...
	void ParticleSystem::ParticleAlphaFromTex(std::string const & tex_name)
	{
		...
		checked_cast<RenderParticles&>(this->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).ParticleAlphaFrom(
	...
	void ParticleSystem::ParticleAlphaToTex(std::string const & tex_name)
	{
		...
		checked_cast<RenderParticles&>(this->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).ParticleAlphaTo(
			SyncLoadTexture(tex_name, EAH_GPU_Read | EAH_Immutable));
	}
	...
	void ParticleSystem::ParticleColorFrom(Color const & clr)
	{
		...
		checked_cast<RenderParticles&>(this->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).ParticleColorFrom(clr);
	}

	void ParticleSystem::ParticleColorTo(Color const & clr)
	{
		particle_color_to_ = clr;
		checked_cast<RenderParticles&>(this->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).ParticleColorTo(clr);
	}

	void ParticleSystem::SceneDepthTexture(TexturePtr const & depth_tex)
	{
		checked_cast<RenderParticles&>(this->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).SceneDepthTexture(depth_tex);
	}
