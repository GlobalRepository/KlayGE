...
namespace KlayGE
{
	...
	void MtlEditorCore::OnCreate()
	{
		...
		main_light_node_->OnMainThreadUpdate().Connect([this](SceneNode& node, float app_time, float elapsed_time) {
			KFL_UNUSED(app_time);
	...
	void MtlEditorCore::SkyboxName(std::string const & name)
	{
		...
		if (!skybox_name_.empty())
		{
			...
			if (!!c_tex)
			{
				checked_cast<RenderableSkyBox&>(skybox_->FirstComponentOfType<RenderableComponent>()->BoundRenderable())
					.CompressedCubeMap(y_tex, c_tex);
	...
	void MtlEditorCore::UpdateSelectedMesh()
	{
		if (selected_obj_ > 0)
		{
			...
			checked_cast<RenderableLineBox&>(selected_bb_->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).SetBox(obb);
