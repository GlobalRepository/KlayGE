...
void TessellationApp::TessellationOnHandler(UICheckBox const & sender)
{
	...
	checked_cast<RenderTriangle&>(polygon_->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).TessEnabled(enabled);
...
void TessellationApp::Edge0ChangedHandler(KlayGE::UISlider const & sender)
{
	...
	checked_cast<RenderTriangle&>(polygon_->FirstComponentOfType<RenderableComponent>()->BoundRenderable()).TessFactors(tess_factor_);
